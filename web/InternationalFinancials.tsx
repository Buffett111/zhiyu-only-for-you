...
export function InternationalFinancials({ security, reports, progress, target }: { security: Security; reports: FinancialReport[]; progress?: ContentProgress; target: TranslationTarget }) {
 ...
 return <div className="international-financials">
  ...
  {!report ? <div className="financial-empty"><h3>{progress?.status==='error'?'財報來源暫時無法取得':'財報資料準備中'}</h3><p>加入追蹤後會下載來源可提供的季度及年度報表；缺項不補成零。</p></div> : <>
   ...
   <p className="report-unit">已保存 {available.length} {basis==='quarter'?'季':'個年度'}：{available.at(-1)?.periodEnd} ～ {available[0]?.periodEnd}。向來源查詢近 10 年，實際涵蓋以上列期間為準；已取得財報會持續保留，缺少的期別不代表零。</p>
   <details className="report-history"><summary>歷年趨勢比較</summary><p className="report-unit">各期金額為報告幣別百萬，EPS 為每股金額；不同幣別不直接比較。季度與年度分開呈現，期間缺項不推估。</p><div className="report-history-scroll" tabIndex={0} role="region" aria-label="歷年財報比較表"><table className="financial-table"><thead><tr><th>期末</th><th>幣別</th><th>營收</th><th>淨利</th><th>EPS</th><th>營業現金流</th><th>自由現金流</th></tr></thead><tbody>{available.map(row=><tr key={row.periodEnd}><th>{row.periodEnd}</th><td>{row.currency}</td><td>{amount(row.revenue)}</td><td>{amount(row.netIncome)}</td><td>{number(row.eps)}{row.eps!==null&&<small>（{row.epsType==='basic'?'基本':'稀釋'}）</small>}</td><td>{amount(row.operatingCashFlow)}</td><td>{amount(row.freeCashFlow)}</td></tr>)}</tbody></table></div></details>
