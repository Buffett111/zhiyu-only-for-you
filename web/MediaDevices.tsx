...
export function MediaDevices({onSync}:{onSync:()=>void}){
  ...
  return <section className="card media-extension"><div className="media-extension-title"><div><span className="eyebrow">YOUTUBE CAPTURE</span><h2>YouTube 觀看紀錄，自動回到知隅。</h2><p>授權一次，自動開啟觀看紀錄頁、持續載入 HTML，回補多年歷史並同步新觀看。</p></div><a className="button primary" href="/downloads/zhiyu-youtube-extension.zip" download><Download size={17}/>下載 Chrome 擴充功能</a></div>
    <ol className="media-setup"><li><strong>安裝擴充功能</strong><span>下載並解壓縮 ZIP，在 Chrome 的擴充功能管理頁開啟「開發人員模式」，選「載入未封裝項目」及解壓縮後的資料夾。</span></li><li><strong>授權並確認帳號</strong><span>開啟「知隅 YouTube Capture」，按「授權並連接知隅」，允許 YouTube 存取，再確認保存資料的知隅帳號。</span></li><li><strong>自動擷取歷史</strong><span>擴充功能會開啟 YouTube 觀看紀錄分頁。請確認 YouTube 帳號正確並保留分頁；它會持續捲動、分批保存，直到來源沒有更多紀錄。</span></li></ol>
    <p className="media-note"><ShieldCheck size={13}/> 只存 YouTube 影片紀錄，不讀 Chrome 全站瀏覽歷史。數年回補可能需要較長時間；中斷會標示未完成，可重新回補並去重。手機可查看已同步結果；擷取需桌面 Chrome 執行。</p>
