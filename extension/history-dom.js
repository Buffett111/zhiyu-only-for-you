// Adapted from urTube (MIT); see THIRD_PARTY_NOTICES.md.
(() => {
  ...
  function parseHistoryDateLabel(text, now = new Date()) {
    ...
    if (/^(今天|今日|오늘|today)$/i.test(value)) return iso(today);
    if (/^(昨天|昨日|어제|yesterday)$/i.test(value)) return iso(new Date(today.getTime() - 86_400_000));
  ...
  function dateForLockup(root, now = new Date()) {
    ...
    const label = section?.querySelector?.('[id="title"]')?.textContent;
    return parseHistoryDateLabel(label, now);
  }

  function progressFromLockup(root) {
    const link = root.querySelector('h3 a[href*="/watch?v="], a#video-title[href*="/watch?v="], a[href*="/shorts/"]');
    if (!link) return null;
  ...
  function collectProgress(documentRoot = document) {
    return collectProgressFromRoots(documentRoot.querySelectorAll('yt-lockup-view-model, ytd-video-renderer, ytd-reel-item-renderer, ytm-shorts-lockup-view-model'));
  }

  function compactHistorySections(documentRoot = document, maximum = 80) {
    const sections = [...documentRoot.querySelectorAll(
      'ytd-item-section-renderer:not([data-zhiyu-compacted])'
    )];
    ...
    for (const section of expired) {
      ...
      section.setAttribute('data-zhiyu-compacted', '');
  ...
  globalThis.zhiyuYoutubeHistory = {
