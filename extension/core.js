...
export function normalizeEvent(value){
  ...
  return {videoId:value.videoId,title:String(value.title||'YouTube 影片').slice(0,500),channel:typeof value.channel==='string'?value.channel.slice(0,160):null,watchedAt:time.toISOString(),actualSeconds:Number.isFinite(value.actualSeconds)?Math.max(0,Math.min(86400,Math.floor(value.actualSeconds))):null,precision:value.precision==='day'?'day':'exact'};
}
export function mergeQueue(queue,incoming){
  const map=new Map(queue.map(e=>[eventKey(e),e]));
  for(const raw of incoming){const e=normalizeEvent(raw);if(!e)continue;const key=eventKey(e),old=map.get(key);if(!old&&map.size>=MAX_QUEUE)throw Error('本機待同步紀錄已滿；恢復知隅連線後會繼續回補。');map.set(key,old?{...e,channel:e.channel??old.channel,actualSeconds:e.actualSeconds===null?old.actualSeconds:Math.max(e.actualSeconds,old.actualSeconds??0)}:e);}
  return [...map.values()];
}
