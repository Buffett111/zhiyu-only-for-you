...
搜尋不預先保存美日個股，加入追蹤才下載行情、財報與新聞。同一股票在多個清單共用資料；停用模組或移除最後一位追蹤者後停止擷取。台股保留必要的搜尋目錄，整批來源僅保存已追蹤行情。公開市場快取有保存期限，不會無限累積。
...
本機版本已通過 91 項測試、正式建置與真實資料管線驗證；完整範圍見[驗證紀錄](docs/verification.md)。公開部署需自行設定 Cloudflare 帳號、Access 邀請名單及本機 Tunnel；程式庫不包含可共用的服務帳號或部署憑證。
