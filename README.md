...
給自己與親友的個人資訊角落。財經模組支援台股、美股與日股的股票／ETF 收盤追蹤：私人自選清單、原幣日線與每天 20:30 的摘要。台股有月營收、一般業財報與新聞公告；ETF 消息可切換本身、成分股及市場背景；美日有季度／年度財報摘要與來源關聯新聞。
...
新增 **urTube 影音分析模組**：Chrome 擴充功能自動開啟 YouTube 觀看紀錄頁，動態載入 HTML 回補多年歷史，查看頻道、主題與觀看趨勢；私人資料保存在本機，支援匯出與按需 AI 分類。使用方式見[影音模組文件](docs/media-module.md)。
...
本機版本已通過 130 項測試、正式建置與真實資料管線驗證；完整範圍見[驗證紀錄](docs/verification.md)。公開部署需自行設定 Cloudflare 帳號、Access 邀請名單及本機 Tunnel；程式庫不包含可共用的服務帳號或部署憑證。
