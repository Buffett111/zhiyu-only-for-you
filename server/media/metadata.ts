...
async function channelIdentity(url:string,request:typeof fetch){
 ...
 const task=(async()=>{
  const r=await request(url,{signal:AbortSignal.timeout(15000),redirect:'error'});if(!r.ok||!r.body)throw Error('Channel identity temporarily unavailable');
...
export async function enrichMediaMetadata(pool:Pool,userId:string,fetchAuthor=fetchVideoAuthor,limit=60){
 ...
 for(let start=0;start<pending.length;start+=3){
  const results=await Promise.all(pending.slice(start,start+3).map(async row=>{
   try{return {id:row.video_id,...await fetchAuthor(row.video_id)};}catch{return {id:row.video_id,status:'error' as const,title:null,channel:null,channelKey:null};}
  }));
  ...
  // A provider failure must not produce thousands of immediate retries.
  if(errors>=3)break;
 }
 return {ready,errors};
}
