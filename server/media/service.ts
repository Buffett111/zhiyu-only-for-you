...
export async function importMedia(pool:Pool,userId:string,bytes:Uint8Array,options:{guard?:(client:PoolClient)=>Promise<void>;source?:string;trackImport?:boolean}={}){
  ...
  try{
    ...
    for(let offset=0;offset<parsed.events.length;offset+=2000){
      await client.query(`INSERT INTO media_events(user_id,event_id,video_id,title,channel,watched_at,actual_seconds,precision,topics,topic_source,source,duration_seconds,progress_percent,resume_seconds)
        SELECT $1,e->>'eventId',e->>'videoId',e->>'title',e->>'channel',(e->>'watchedAt')::timestamptz,(e->>'actualSeconds')::int,e->>'precision',e->'topics',e->>'topicSource',e->>'source',(e->>'durationSeconds')::int,(e->>'progressPercent')::float8,(e->>'resumeSeconds')::int FROM jsonb_array_elements($2::jsonb) e
        ON CONFLICT(user_id,event_id) DO UPDATE SET duration_seconds=COALESCE(EXCLUDED.duration_seconds,media_events.duration_seconds),progress_percent=GREATEST(EXCLUDED.progress_percent,media_events.progress_percent),resume_seconds=GREATEST(EXCLUDED.resume_seconds,media_events.resume_seconds),title=EXCLUDED.title,channel=COALESCE(EXCLUDED.channel,media_events.channel),actual_seconds=GREATEST(EXCLUDED.actual_seconds,media_events.actual_seconds),
...
export async function mediaSummary(pool:Pool,userId:string,range:typeof MEDIA_RANGES[number],config:Config):Promise<MediaSummary>{
  ...
  const [totals,counts,channels,topics,daily,hourly,imports,unclassified]=await Promise.all([
    ...
    pool.query(`SELECT count(*)::int selected,count(DISTINCT video_id)::int videos,count(DISTINCT (watched_at AT TIME ZONE 'Asia/Taipei')::date)::int days,sum(actual_seconds)::float8 seconds,count(actual_seconds)::int timed,sum(estimated_seconds)::float8 estimated,sum(estimated_seconds) FILTER(WHERE actual_seconds IS NULL)::float8 progress_seconds,count(estimated_seconds)::int estimated_events,count(*) FILTER(WHERE actual_seconds IS NULL AND estimated_seconds IS NOT NULL AND (progress_percent IS NOT NULL OR resume_seconds IS NOT NULL))::int progress_events,count(*) FILTER(WHERE actual_seconds IS NULL AND duration_seconds IS NOT NULL AND progress_percent IS NULL AND resume_seconds IS NULL)::int duration_only,count(*) FILTER(WHERE jsonb_array_length(topics)>0)::int classified FROM media_events WHERE ${base}`,[userId,days]),
