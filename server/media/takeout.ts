// Adapted from urtube contributors (MIT); see THIRD_PARTY_NOTICES.md.
import { createHash } from 'node:crypto';
import * as cheerio from 'cheerio';
interface YoutubeWatchInput {eventId:string;videoId:string|null;title:string;url:string;channelId:string|null;channelTitle:string|null;channelUrl:string|null;watchedAt:string;actualWatchedSeconds:number|null;activityType:'video'|'post'|'other';precision?:'day'|'exact';}
...
function videoIdFromUrl(value: string): string | null {
  try {const url=new URL(value);if(!['https:','http:'].includes(url.protocol))return null;
    const id=url.hostname==='youtu.be'?url.pathname.slice(1):['youtube.com','www.youtube.com','m.youtube.com','music.youtube.com'].includes(url.hostname)?url.pathname==='/watch'?url.searchParams.get('v'):url.pathname.match(/^\/(?:shorts|live)\/([^/]+)$/)?.[1]:null;
...
export function parseWatchActivities(items: unknown[]): YoutubeWatchInput[] {
  ...
  for (const raw of items) {
    ...
    const activityType = (videoIdFromUrl(text(item.titleUrl)) || /^(Watched\s+|已觀看|觀看了|观看了)/i.test(rawTitle))
      ? 'video'
    ...
    const title = rawTitle.replace(/^(?:(?:Watched|Viewed)\s+|已觀看\s*|觀看了\s*|观看了\s*)/i, '').trim()
...
export function parseHtml(bytes: Uint8Array, name: string): TakeoutActivity[] {
