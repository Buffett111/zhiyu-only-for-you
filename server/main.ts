...
await boss.start();
for (const name of QUEUES) await boss.createQueue(name, { policy: name === 'digest.generate' ? 'stately' : 'exclusive', retryLimit: name === 'international.sync' ? 0 : 2, retryDelay: 120, retryBackoff: true, expireInSeconds: 3600 });
